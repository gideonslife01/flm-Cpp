...
int main() {
    ...
    // https 라우터❗️
    Router server3_router("../public3");
    server3_router.add_route("GET","/test1", "test1.html");
    server3_router.add_route("POST","/test2", "test2.html");
    ...
    // --- 서버 2 설정 (6080 포트) --- //
    // --- Server 2 Configuration (Port 6080) --- //
    http::Server server2("127.0.0.1", 6080);
    ...
    // --- 서버 3 설정 (6443 포트) --- ❗️//
    // --- Server 3 Configuration (Port 6443) --- //
    https::Server server3("127.0.0.1", 6443);
    //
    // SSL 초기화 및 인증서,개인키 로드❗️
    ...
    */
    ...
    server3.set_handler([&](const std::string& req) {
        ...
        if (method == "GET" || method == "POST" || method == "PUT" || method == "DELETE" || method == "FETCH") {
            result = server3_router.router(method, path);
        }
        else {
            return std::string(
                "HTTP/1.1 405 Method Not Allowed\r\n"
    ...
    // 네임스페이스 적용 / Apply namespaces ❗️
    std::thread server_thread(&http::Server::start, &server1);
    std::thread server_thread2(&http::Server::start, &server2);

    // https ❗️
    std::thread server_thread3(&https::Server::start, &server3);
