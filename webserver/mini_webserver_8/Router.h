...
class Router {
    ...
    Router(std::string root = "../public") : doc_root(root) {}
     ...
     // 함수 선언 / function declaration - 기존방식❗️
    ...
    // 서버별로 경로를 추가할 수 있는 함수 / A function that can add paths per server //기존 방식❗️
    // void add_route(const std::string& path, const std::string& file_name) {
    //     route_map[path] = file_name;
    // }
    //
    // 메소드 추가 / method added❗️
    ...
    private:
        ...
        //std::map<std::string, std::string> route_map; // ❗️ 기존방식
        //
        // method,path,filename - 추가 ❗️
