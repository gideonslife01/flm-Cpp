...
#include <atomic>
#include <functional>
// OpenSSL 헤더 포함 (HTTPS 처리를 위해 필수)
#include <openssl/ssl.h>
#include <openssl/err.h>
#include <vector>
#include <thread>

// 네임스페이스 설정 / namespace setting
namespace https {
    class Server {
    public:
        // 생성자: 서버 초기화 (IP, Port)
        ...
        // 커맨드 로직 구현 / implement command logic
        void run_cmd();

        // -- 브라우저 메세지 바꾸기 -- //
        // string(요청)을 받아서 string(응답내용)을 뱉는 함수를 담는 그릇
        //
        // -- Changing browser messages -- //
        ...
        // SSL 초기화 및 인증서, 개인키 로드 함수 - https
        bool initialize_ssl(const std::string& cert_file, const std::string& key_file);
    ...
    private:
        ...
        // https
        // SSL_CTX 포인터: OpenSSL TLS 컨텍스트 관리
        SSL_CTX* ssl_ctx = nullptr;

        // 스레드 안전 종료를 위해 스레드 저장할 백터
        // Vector to store threads for thread-safe termination
