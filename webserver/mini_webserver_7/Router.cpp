...
// 이전 생성자 / previous constructor❗️
// Router::Router() {

// }
//
// 주석처리 가능 ,신규추가 / Commentable, Newly added❗️
...
std::pair<std::string, std::string> Router::router(const std::string& request) {
    ...
    // --  기존설정 / Existing settings -- ❗️//
    // 기본 문서 루트 설정 / Set default document root
    // 변수는 헤더에 추가함 / Variables are added to the header❗️
    ...
    // }else if(path == "/aloy"){
    //     full_path = doc_root + "/aloy.html";
    // }else if(path == "/silverhand"){
    //     full_path = doc_root + "/silverhand.html";
    // }else{
    //     full_path = doc_root + "/index.html";
    // }
