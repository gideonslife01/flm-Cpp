...
#include <functional>
#include <string> // ❗️
#include <openssl/ssl.h> //❗️
#include <openssl/err.h> //❗️
...
void https::Server::run_listener() {
    ...
    while (running.load()) {
       ...
       // struct sockaddr_in client_address; // C Style
        sockaddr_in client_address{}; //❗️ -> C++ Style
...
void https::Server::handle_client(int client_socket) {
    ...
    //char buffer[1024];
    char buffer[4096] = {0}; // 안정적인 수신을 위해 버퍼 확대 / Buffer enlargement for stable reception❗️
    // -- debuging --
   ...
   int bytes_read = SSL_read(ssl, buffer, sizeof(buffer) - 1); //❗️
   if (bytes_read > 0) {
        buffer[bytes_read] = '\0';
        ...
        // 중복으로 삭제 / Delete as duplicate❗️
        // std::string response = html_content; // main.cpp에서 넘어온 헤더 적용 / Apply header passed from main.cpp
        ...
        // 오류 또는 연결 종료 처리 - 추가함 / Error or connection termination handling - added❗️
        ERR_print_errors_fp(stderr);
    }
