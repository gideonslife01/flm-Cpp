...
int main() {
    ...
    Router server3_router("../public");
    server3_router.add_route("GET","/api", "index.html");

    // virtusl host 라우터 / virtual host router
    ...
    // virtual host http(5080) server host map
    ...
    // virtual host https(6443) server host map
    ...
    // virtual host http(8080) server host map❗️
    ...
    server1.set_handler([&](const std::string& req) {
        ...
        // -- 메인호스트,가상호스트 라우터 선택 / Select Main Host, Virtual Host, and Router == //
        ...
        // 요청 헤더 파싱(req stream의 값을 읽어서 line 변수에 한 줄씩 복사)
        ...
        // std::string::npos : 찾는 문자열이 없을 때 반환되는 특별한 값.
        // std::string::npos : A special value returned when the string being searched for is not found.
        if (pos != std::string::npos) {
            host_normalized = host.substr(0, pos);
        }

        // 라우터 선택: 해당 호스트의 라우터 찾기, 없으면 기본 라우터
        ...
        if (method == "GET" || method == "POST" || method == "PUT" || method == "DELETE" || method == "FETCH") {
            ...
            //
            // virtual host 적용 / apply virtual host
    ...
    */
    ...
    server2.set_handler([&](const std::string& req) {
        ...
        if ((method == "GET" || method == "POST") &&
            ...
            // /api → / 로 변경 (백엔드가 루트 경로로 받도록)
            // Change /api to / (so the backend receives it as the root path)
            if (path == "/api") {
                size_t pos = modified_req.find(" /api ");
                if (pos != std::string::npos) {
                    modified_req.replace(pos, 5, " /");
    ...
    server3.set_handler([&](const std::string& req) {
        ...
        // 라우터 선택: 해당 호스트의 라우터 찾기, 없으면 기본 라우터
        // Select Router: Find the router for the host, if none exists, use the default router
        Router* router_ptr = &server3_router; // 기본값 / default value
        auto it = vhost_reverse_routers.find(host_normalized);
        if (it != vhost_reverse_routers.end()) {
            router_ptr = it->second;
        }
    ...
    std::thread server_thread3(&http::Server::start, &server3); // reverse proxy❗️
