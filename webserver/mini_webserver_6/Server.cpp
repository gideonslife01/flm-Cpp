...
void Server::run_listener() {
    ...
    while (running.load()) {
        ...
        std::this_thread::sleep_for(std::chrono::milliseconds(100)); // 부하 방지를 위해 대기 / Standby to prevent overload
        ...
        std::thread client_thread([this, client_socket]() {
            this->handle_client(client_socket); //외부에서 메세지 바꾸기 / Change message externally
        });
